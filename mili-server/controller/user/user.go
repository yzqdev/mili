...
func sendMail(action string, title string, curTime int64, user model.User, c *gin.Context) {
	...
	//fmt.Println(content)
	SendQQEmail(user.Email, title, content)
	//mail.SendMail(user.Email, title, content)
}

func SendQQEmail(toEmail, title, content string) {
	e := email.NewEmail()
	//设置发送方的邮箱
...
func Signin(c *gin.Context) {
	...
	var sql string
	color.Yellow.Println("hhhhhhhhhhhhhhhhh")
	color.Yellow.Println(c.Query("loginType"))
	if c.Query("loginType") == "email" {
		...
		if err := c.ShouldBindWith(&usernameLogin, binding.JSON); err != nil {
			color.Yellow.Println("shibaileadfsfsdfdf+__________")
			fmt.Println(err.Error())
	...
	if user.CheckPassword(password) {
		...
		color.Yellow.Println("到达这里")
		color.Yellow.Println(tokenString)
		color.Cyan.Println(user)
